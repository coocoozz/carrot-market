...
export default function Enter() {
  ...
  const onEnterValid = (data: EnterForm) => {
    enter(data);
  };

  const onEnterInvalid = (error: FieldErrors) => {
    console.log(error);
  };
      ...
      <div className="mt-12">
            ...
            >
              <Input
                ...
                register={tokenRegister("token", {
                  required: "need to specify token",
                })}
              />
              <Button text={tokenLoading ? "Submitting..." : "Confirm Token"} />
            </form>
          </>
        ) : (
          <>
            ...
            <form
              className="flex flex-col mt-8 space-y-4"
              onSubmit={enterHandleSubmit(onEnterValid, onEnterInvalid)}
            >
              {method === "email" ? (
                <Input
                  name="email"
              ...
              {method === "email" ? (
                <Button
                  text={enterLoading ? "Submitting..." : "Get login link"}
                />
              ) : null}
              {method === "phone" ? (
                <Button
                  text={
                    enterLoading ? "Submitting..." : "Get one-time password"
                  }
                />
              ) : null}
            </form>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-2 gap-3 mt-6">
            <button className="flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:text-gray-600">
              <svg
                className="w-5 h-5"
                aria-hidden="true"
                fill="currentColor"
